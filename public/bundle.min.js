...
var React=require("react/addons"),ReactCSSTransitionGroup=React.addons.CSSTransitionGroup,axios=require("axios"),timelineJsonData=require("./data/timeline.json"),HoH=require("./js/dispatcher.js");window.app=function(){for(var e={"JSON decoding":window.JSON,"the selectors API":document.querySelector,"DOM level 2 events":window.addEventListener,"the HTML5 history API":window.history.pushState},t=e.length-1;t>=0;t--)if(!e[t])return alert("Sorry, but your browser does not support "+feature+" so this app won't work properly.");if("/"!==window.location.pathname){var o=window.location.pathname.split("/");console.log(o);var r={year:o[1],position:o[2],name:o[3]}}return React.render(React.createElement(HoH,{timeline:timelineJsonData,initparams:r}),document.getElementById("hoh"))}();
